...
export class S3Storage implements BinaryStorage {
  ...
  async get(key: string): Promise<Buffer> {
    ...
    )) as { ContentLength?: number; Body?: { transformToByteArray: () => Promise<Uint8Array>; destroy?: () => void } };
    if (!result.Body || !result.ContentLength || result.ContentLength > 5_000_000) {
      result.Body?.destroy?.();
      throw new Error("Stored diagram must be a non-empty image below 5 MB.");
    }
    const bytes = Buffer.from(await result.Body.transformToByteArray());
    if (bytes.length > 5_000_000) throw new Error("Stored diagram exceeds 5 MB.");
    return bytes;
  }
