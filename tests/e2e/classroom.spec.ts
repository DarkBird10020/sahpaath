...
test("browser-speech captions ask for permission first, explain a refusal, and caption what is heard", async ({ page }) => {
  ...
  await page.reload();
  await page.getByRole("button", { name: "Captions", exact: true }).click();
  await page.getByRole("button", { name: "Use browser speech instead" }).click();
